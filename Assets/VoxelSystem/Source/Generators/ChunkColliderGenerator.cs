...
namespace VoxelSystem.Generators
{
    public class ChunkColliderGenerator
    {
        ...
        internal struct ChunkColliderJob : IJob
        {
            ...
            {
                #region Allocations
                NativeArray<float3> Vertices = new(8, Allocator.Temp)
                {
                    // top
                ...
                NativeArray<int> FaceVerticeIndex = new(24, Allocator.Temp)
                {
                    [0] = 4,
                    [1] = 5,
                    [2] = 1,
                ...
                NativeArray<int> FaceIndices = new(6, Allocator.Temp)
                {
                    [0] = 0,
                    [1] = 3,
                    [2] = 2,
                ...
                NativeArray<int> SidesNeighbourVerticesIndices = new(8, Allocator.Temp)
                {
                    // on z axis
                    [0] = -4 * (chunkWidth) + 3,
                    [1] = -4 * (chunkWidth) + 2,
                ...
                NativeArray<int> SideFaceIndices = new(24, Allocator.Temp)
                {
                    //back
                ...
                for (int z = 1; z <= chunkWidth; z++)
                {
                    ...
                    {
                        float3 voxelPos = new(x - 1, (int)(RWStructs.GetSolid(heightMaps[GetMapIndex(x, z)])) - 1, z - 1);
